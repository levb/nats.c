...
_createSrv(natsServer **newSrv, natsPool *pool, char *url, bool implicit, const char *tlsName)
{
    ...
    natsServer *srv = (natsServer *)natsPool_Alloc(pool, sizeof(natsServer));
    if (srv == NULL)
    ...
    {
        srv->tlsName = natsPool_StrdupC(pool, tlsName);
        if (srv->tlsName == NULL)
...
_addURLToServers(natsServers *servers, char *sURL, bool implicit, const char *tlsName)
{
    ...
    {
        DUP_STRING_POOL(s, servers->pool, servers->user, srv->url->username);
        if ((s == NATS_OK) && !nats_isStringEmpty(srv->url->password))
            DUP_STRING_POOL(s, servers->pool, servers->pwd, srv->url->password);
        if (s != NATS_OK)
            return NATS_UPDATE_ERR_STACK(s);
    }
    ...
    if (servers->size + 1 > servers->cap)
    {
        ...
        newArray = (natsServer **)natsPool_Alloc(servers->pool, newCap * sizeof(char *));
...
natsServers_Create(natsServers **newServers, natsPool *pool, natsOptions *opts)
{
    ...
    IFOK(s, natsPool_AllocS((void**)&servers, pool, sizeof(natsServers)));
    IFOK(s, natsPool_AllocS((void**)&servers->srvrs, pool, c * sizeof(natsServer *)));
    ...
    if (s == NATS_OK)
    {
        servers->pool = pool;
        servers->cap = c;
        servers->randomize = !opts->noRandomize;
    }
    ...
    else if ((s == NATS_OK) && (servers->size == 0))
    {
...
natsServers_GetNextServer(natsServers *servers, natsOptions *opts, const natsServer *cur)
{
    ...
    if ((opts->maxReconnect < 0) || (s->reconnects < opts->maxReconnect))
    {
                       ...
                       const natsUrl *curUrl, const char **urls, int urlCount, const char *tlsName, bool *added)
{
    ...
    *added = false;
    ...
    infoURLs = natsPool_Alloc(servers->pool, sizeof(const char *) * urlCount);
    if (infoURLs == NULL)
        return nats_setDefaultError(NATS_NO_MEMORY);
    ...
    if (s == NATS_OK)
    {
        for (int i=0; (s == NATS_OK) && (i < infoURLCount); i++)
        {
        ...
        if ((s == NATS_OK) && *added && servers->randomize)
