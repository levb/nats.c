...
void nats_ProcessReadEvent(natsConnection *nc)
{
    ...
    IFOK(s, nats_getReadBuffer(&rbuf, nc->opPool));
    if (STILL_OK(s))
    {
        ...
        // loop.
        haveRead.bytes = nats_readBufferEnd(rbuf);
        s = natsSock_Read(&(nc->sockCtx),
                          nats_readBufferEnd(rbuf),
                          nats_readBufferAvailable(&nc->opts->mem, rbuf),
                          &haveRead.len);
    ...
    while ((STILL_OK(s)) && (rbuf != NULL) && (nats_readBufferUnreadLen(rbuf) > 0))
    {
        ...
        s = natsConn_parseOp(nc, rbuf->readFrom, nats_readBufferEnd(rbuf), &consumedByParser);
...
natsConn_processInfo(natsConnection *nc, nats_JSON *json)
{
    ...
    default:
        return NATS_UPDATE_ERR_STACK(nats_setErrorf(NATS_PROTOCOL_ERROR,
    ...
    if (s != NATS_OK)
        s = nats_setErrorf(s, "Invalid protocol: %s", nats_GetLastError(NULL));
...
natsStatus natsConn_processPong(natsConnection *nc)
{
    ...
    if (nc->pingsOut == 0)
        return nats_setError(NATS_PROTOCOL_ERROR, "Received unexpected PONG");
