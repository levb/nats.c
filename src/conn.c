...
natsConnection_Connect(natsConnection **newConn, natsOptions *options)
{
    ...
    IFOK(s, natsServers_Create(&(nc->servers), nc->lifetimePool, nc->opts));
    if (s == NATS_OK)
        nc->cur = natsServers_Get(nc->servers, 0);
    IFOK(s, _connectTCP(nc));
    IFOK(s, nc->opts->evCbs.attach(&(nc->el.data),
                                   nc->opts->evLoop,
                                   nc,
                                   (int)nc->sockCtx.fd));
    if (s == NATS_OK)
...
_createConnectionObject(natsConnection **newConn)
{
    ...
    CONNLOGf("_createConnectionObject: created natsConnection: %p", (void *)nc);
    *newConn = nc;
...
//     if ((s == NATS_OK) && (!natsString_EqualC(&op, _INFO_OP_)))
//     {
...
natsConn_processInfo(natsConnection *nc, nats_JSON *json)
{
    ...
    IFOK(s, natsPool_AllocS((void **)&info, nc->lifetimePool, sizeof(natsServerInfo)));

    IFOK(s, nats_JSONDupStr(json, nc->lifetimePool, "server_id", &info->id));
    IFOK(s, nats_JSONDupStr(json, nc->lifetimePool, "version", &(info->version)));
    IFOK(s, nats_JSONDupStr(json, nc->lifetimePool, "host", &(info->host)));
    ...
    IFOK(s, nats_JSONRefStr(json, "client_ip", &(info->clientIP)));
...
//         if (nats_isStringEmpty(user) && nats_isStringEmpty(token) && (nc->servers->user != NULL))
//         {
